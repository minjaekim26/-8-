...
def get_kci_factor_display_df(base_dir: Optional[str] = None) -> pd.DataFrame:
    ...
    display["p값"] = display["p값"].apply(
        lambda p: "< .001" if isinstance(p, str) and p.startswith("0.001") else p
    )
    return display
...
def set_profile_from_form(
    ...
    *,
    ...
    structured["profile"] = {
        ...
        "Sleep_Duration": 7.0,
        "Daily_Steps": 5000,
        "Happiness_Level": estimate_happiness_level(7.0, 6.5, 5.0),
    }
